...
async function fixPrismaSchema(modelName) {
    ...
    for (const line of lines) {
        ...
        if (insideModel) {
            if (line.includes("tenantId"))
                foundTenantId = true;
            if (line.includes("@@index([tenantId])"))
                indexAdded = true;
            if (line.trim() === "}") {
                if (!foundTenantId) {
...
async function fixServiceInjection(filePath) {
...
async function fixPrismaQuery(filePath, line) {
    let content = await fs.readFile(filePath, "utf-8");
    const lines = content.split("\n");
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        switch (request.params.name) {
            ...
            case "fix_prisma_schema": return await fixPrismaSchema(request.params.arguments?.modelName);
            case "fix_service_injection": return await fixServiceInjection(request.params.arguments?.filePath);
            case "fix_prisma_query": return await fixPrismaQuery(request.params.arguments?.filePath, request.params.arguments?.line);
