...
interface QuoteItem extends DocumentItem { }
...
export default function YeniSatisTeklifiPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        setProducts((productsRes.data.data || []).map((p: any) => ({
          ...
          stokKodu: p.code,
          stokAdi: p.name,
          satisFiyati: Number(p.salePrice) || 0,
          kdvOrani: Number(p.vatRate) || 20,
          barkod: p.barcode,
          birim: p.unit || 'ADET'
        })));

        const quotesRes = await axios.get('/quotes', { params: { quoteType: 'SALE', limit: 1 } });
        ...
        addTab({ id: 'quotes-sales-new', label: 'Yeni Satış Teklifi', path: '/quotes/sales/yeni' });
      } catch (error: any) {
  ...
  const handleSave = async () => {
    try {
      ...
      setTimeout(() => router.push('/quotes/sales'), 1500);
    } catch (e: any) {
    ...
    <StandardPage>
      <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography variant="h5" fontWeight="800">Yeni Satış Teklifi</Typography>
