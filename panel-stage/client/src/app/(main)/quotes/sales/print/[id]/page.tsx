...
export default function TeklifPrintPage() {
  ...
  const fetchTeklif = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/teklif/${id}`);
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Teklif-${teklif?.teklifNo}`,
  });
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Teklif-${teklif?.teklifNo}.pdf`);
    } catch (error) {
    ...
    <Box sx={{
      ...
      <Box sx={{
        ...
        <div ref={printRef} style={{ minWidth: 'fit-content' }}>
          {template === 'classic' ? (
            <ClassicTemplate teklif={teklif} paperSize={paperSize} formatDate={formatDate} formatMoney={formatMoney} />
          ) : (
            <ModernTemplate teklif={teklif} paperSize={paperSize} formatDate={formatDate} formatMoney={formatMoney} />
          )}
        </div>
      </Box>
    ...
    >
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
          ...
          <Box sx={{ textAlign: 'right' }}>
            ...
            <Typography variant="body2" sx={{ mt: 1 }}>
              <strong>Teklif No:</strong> {teklif.teklifNo}
            </Typography>
            <Typography variant="body2">
              <strong>Tarih:</strong> {formatDate(teklif.tarih)}
            </Typography>
            <Typography variant="body2">
              <strong>Geçerlilik Tarihi:</strong> {teklif.gecerlilikTarihi ? formatDate(teklif.gecerlilikTarihi) : "-"}
            </Typography>
          </Box>
        </Stack>
      </Box>
      ...
      <Box sx={{ mb: 3, p: 2, bgcolor: '#f8f9fa', borderRadius: 1 }}>
        ...
        <Stack direction="row" spacing={3}>
          <Box sx={{ flex: 1 }}>
            <Typography variant="body2"><strong>Müşteri Kodu:</strong> {teklif.cari.cariKodu}</Typography>
            <Typography variant="body2"><strong>Ünvan:</strong> {teklif.cari.unvan}</Typography>
          </Box>
          <Box sx={{ flex: 1 }}>
            {teklif.cari.telefon && (
              <Typography variant="body2"><strong>Telefon:</strong> {teklif.cari.telefon}</Typography>
            )}
            {teklif.cari.adres && (
              <Typography variant="body2"><strong>Adres:</strong> {teklif.cari.adres}</Typography>
            )}
          </Box>
        </Stack>
      </Box>
      ...
      <TableContainer sx={{
        ...
        <Table size="small" sx={{
          ...
          <TableBody>
            {teklif.kalemler.map((kalem, index) => (
        ...
        <Box sx={{ width: paperSize === 'A4' ? '250px' : '180px' }}>
          <Stack spacing={0.5} sx={{ p: 2, bgcolor: '#f8f9fa', borderRadius: 1 }}>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography sx={{ fontSize: '9pt' }}>{formatMoney(teklif.toplamTutar)}</Typography>
            </Stack>
            {teklif.iskonto > 0 && (
              <Stack direction="row" justifyContent="space-between">
                <Typography sx={{ fontSize: '9pt' }}>İskonto:</Typography>
                <Typography sx={{ fontSize: '9pt', color: '#dc2626' }}>-{formatMoney(teklif.iskonto)}</Typography>
              </Stack>
            )}
            <Stack direction="row" justifyContent="space-between">
              <Typography sx={{ fontSize: '9pt' }}>KDV:</Typography>
              <Typography sx={{ fontSize: '9pt' }}>{formatMoney(teklif.kdvTutar)}</Typography>
            </Stack>
            <Divider />
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography sx={{ fontWeight: 'bold', fontSize: '10pt', color: '#191970' }}>
                {formatMoney(teklif.genelToplam)}
              </Typography>
            </Stack>
          </Stack>
        </Box>
      ...
      {teklif.aciklama && (
        <Box sx={{ mt: 2, p: 1.5, bgcolor: '#f8f9fa', borderRadius: 1 }}>
          ...
          <Typography variant="body2">{teklif.aciklama}</Typography>
        </Box>
      )}
          ...
          <Stack direction="row" spacing={3}>
            <Box sx={{ flex: 1 }}>
              <Typography variant="body2" sx={{ color: '#666' }}>
                <strong>Müşteri Kodu:</strong> {teklif.cari.cariKodu}
              </Typography>
            </Box>
            <Box sx={{ flex: 1 }}>
              {teklif.cari.telefon && (
                <Typography variant="body2" sx={{ color: '#666' }}>
                  <strong>Telefon:</strong> {teklif.cari.telefon}
                </Typography>
              )}
              {teklif.cari.adres && (
                <Typography variant="body2" sx={{ color: '#666' }}>
                  <strong>Adres:</strong> {teklif.cari.adres}
                </Typography>
              )}
            </Box>
          </Stack>
          ...
          <Box sx={{
            ...
            <Stack spacing={0.5}>
              ...
              {teklif.iskonto > 0 && (
                <Stack direction="row" justifyContent="space-between">
                  <Typography sx={{ fontSize: '9pt' }}>İskonto</Typography>
                  <Typography sx={{ fontSize: '9pt' }}>-{formatMoney(teklif.iskonto)}</Typography>
                </Stack>
              )}
              <Stack direction="row" justifyContent="space-between">
                <Typography sx={{ fontSize: '9pt' }}>KDV</Typography>
