...
export function SatisFaturaForm({ faturaId: editFaturaId, onBack }: { faturaId?: string; onBack?: () => void }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [carilerRes, stoklarRes, agentsRes, warehousesRes] = await Promise.all([
          axios.get('/account', { params: { limit: 1000 } }),
          axios.get('/products', { params: { limit: 1000 } }),
          axios.get('/sales-agent'),
        ...
        if (isEdit) {
          ...
          setFormData({
            ...
            aciklama: f.notes || '',
            satisElemaniId: f.salesAgentId || '',
            eScenario: f.eScenario || 'TICARI_FATURA',
            eInvoiceType: f.eInvoiceType || 'SALE',
          ...
          const res = await axios.get(`/orders/${siparisId}`);
          const s = res.data;
          setFormData(prev => ({
            ...prev,
            cariId: s.accountId || '',
          ...
          const res = await axios.get(`/sales-waybills/${irsaliyeId}`);
          const irs = res.data;
          setFormData(prev => ({
            ...prev,
  ...
  const generateFaturaNo = async () => {
    try {
      const res = await axios.get('/code-templates/preview-code/INVOICE_SALES');
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        salesAgentId: formData.satisElemaniId,
      ...
      if (isEdit) {
        ...
        clearDraft('sales'); // Yeni fatura başarıyla kaydedildiğinde taslağı temizle
      }
        ...
        <Box sx={{
          ...
          <Box>
            <Typography variant="h4" fontWeight="bold" sx={{ color: 'var(--foreground)' }}>
              {editFaturaId ? 'Satış Faturası Düzenle' : 'Yeni Satış Faturası'}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Satışlarınızı faturalandırın
            </Typography>
          </Box>
        </Box>
      </Box>
          ...
          <Box>
            <Box sx={{
              ...
              <Button
                ...
                disabled={saving || (isEdit && formData.durum === 'APPROVED')}
                sx={{
