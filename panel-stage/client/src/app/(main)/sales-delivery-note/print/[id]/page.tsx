...
export default function IrsaliyePrintPage() {
  ...
  const [irsaliye, setIrsaliye] = useState<Irsaliye | null>(null);
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `İrsaliye-${irsaliye?.irsaliyeNo}`,
  });
  ...
  return (
    <Box sx={{ bgcolor: '#f5f5f5', minHeight: '100vh', p: 3 }}>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" flexWrap="wrap">
          <Stack direction="row" spacing={2} alignItems="center">
            <Typography variant="h6">Satış İrsaliyesi Önizleme</Typography>
      ...
      <Box sx={{
        ...
        <div ref={printRef}>
          {template === 'classic' ? (
            <ClassicTemplate irsaliye={irsaliye} paperSize={paperSize} formatDate={formatDate} formatMoney={formatMoney} />
          ) : (
            <ModernTemplate irsaliye={irsaliye} paperSize={paperSize} formatDate={formatDate} formatMoney={formatMoney} />
          )}
        </div>
      </Box>
...
function ClassicTemplate({
  ...
  return (
    ...
    >
      {/* Header */}
      <Box sx={{ borderBottom: '3px solid #8b5cf6', pb: 2, mb: 3 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
          <Box>
            <Typography variant="h4" sx={{ color: '#8b5cf6', fontWeight: 'bold', fontSize: paperSize === 'A4' ? '24pt' : '18pt' }}>
              YEDEK PARÇA
            </Typography>
          ...
          <Box sx={{ textAlign: 'right' }}>
            <Typography variant="h5" sx={{ color: '#8b5cf6', fontWeight: 'bold', fontSize: paperSize === 'A4' ? '18pt' : '14pt' }}>
              SATIŞ İRSALİYESİ
            </Typography>
            <Typography variant="body2" sx={{ mt: 1 }}>
              <strong>İrsaliye No:</strong> {irsaliye.irsaliyeNo}
            </Typography>
            <Typography variant="body2">
              <strong>Tarih:</strong> {formatDate(irsaliye.irsaliyeTarihi)}
            </Typography>
            {irsaliye.kaynakSiparis && (
              <Typography variant="body2">
                <strong>Sipariş No:</strong> {irsaliye.kaynakSiparis.siparisNo}
              </Typography>
            )}
          </Box>
        </Stack>
      ...
      <Box sx={{ mb: 3, p: 2, bgcolor: '#f8f9fa', borderRadius: 1 }}>
        <Typography variant="subtitle2" sx={{ fontWeight: 'bold', mb: 1, color: '#8b5cf6' }}>
          MÜŞTERİ BİLGİLERİ
        </Typography>
        <Stack direction="row" spacing={3}>
          <Box sx={{ flex: 1 }}>
            <Typography variant="body2"><strong>Müşteri Kodu:</strong> {irsaliye.cari.cariKodu}</Typography>
            <Typography variant="body2"><strong>Ünvan:</strong> {irsaliye.cari.unvan}</Typography>
          </Box>
          <Box sx={{ flex: 1 }}>
            {irsaliye.cari.telefon && (
              <Typography variant="body2"><strong>Telefon:</strong> {irsaliye.cari.telefon}</Typography>
            )}
            {irsaliye.cari.adres && (
              <Typography variant="body2"><strong>Adres:</strong> {irsaliye.cari.adres}</Typography>
            )}
          </Box>
        </Stack>
      </Box>
        ...
        <Box sx={{ width: paperSize === 'A4' ? '250px' : '180px' }}>
          <Stack spacing={0.5} sx={{ p: 2, bgcolor: '#f8f9fa', borderRadius: 1 }}>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography sx={{ fontSize: '9pt' }}>{formatMoney(irsaliye.toplamTutar)}</Typography>
            </Stack>
            <Stack direction="row" justifyContent="space-between">
              <Typography sx={{ fontSize: '9pt' }}>KDV:</Typography>
              <Typography sx={{ fontSize: '9pt' }}>{formatMoney(irsaliye.kdvTutar)}</Typography>
            </Stack>
            <Divider />
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography sx={{ fontWeight: 'bold', fontSize: '10pt', color: '#8b5cf6' }}>
                {formatMoney(irsaliye.genelToplam)}
              </Typography>
            </Stack>
          </Stack>
        </Box>
    ...
    >
      ...
      <Box sx={{ p: paperSize === 'A4' ? 4 : 2 }}>
        ...
        <Box sx={{
          ...
          <Typography variant="caption" sx={{ color: '#8b5cf6', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1 }}>
            Teslim Edilen
          </Typography>
          ...
          <Stack direction="row" spacing={3}>
            <Box sx={{ flex: 1 }}>
              <Typography variant="body2" sx={{ color: '#666' }}>
                <strong>Müşteri Kodu:</strong> {irsaliye.cari.cariKodu}
              </Typography>
            </Box>
            <Box sx={{ flex: 1 }}>
              {irsaliye.cari.telefon && (
                <Typography variant="body2" sx={{ color: '#666' }}>
                  <strong>Telefon:</strong> {irsaliye.cari.telefon}
                </Typography>
              )}
              {irsaliye.cari.adres && (
                <Typography variant="body2" sx={{ color: '#666' }}>
                  <strong>Adres:</strong> {irsaliye.cari.adres}
                </Typography>
              )}
            </Box>
          </Stack>
        ...
        <TableContainer sx={{ mb: 2 }}>
          <Table size="small" sx={{ '& td, & th': { fontSize: 'inherit', border: 'none', py: 1 } }}>
            <TableHead>
              <TableRow sx={{ borderBottom: '2px solid #8b5cf6' }}>
                <TableCell sx={{ fontWeight: 'bold', color: '#8b5cf6' }}>ÜRÜN</TableCell>
                <TableCell align="center" sx={{ fontWeight: 'bold', color: '#8b5cf6' }}>ADET</TableCell>
                <TableCell align="right" sx={{ fontWeight: 'bold', color: '#8b5cf6' }}>FİYAT</TableCell>
                <TableCell align="right" sx={{ fontWeight: 'bold', color: '#8b5cf6' }}>TOPLAM</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {irsaliye.kalemler.map((kalem, index) => (
