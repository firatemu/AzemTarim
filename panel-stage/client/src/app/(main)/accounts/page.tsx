...
export default function CariPage({
  ...
  const [newCariLoading, setNewCariLoading] = useState(false);
  ...
  const resetForm = async () => {
    ...
    try {
      ...
      console.log('Otomatik kod alınamadı, boş bırakılacak:', error);
    }
  ...
  // Yeni Cari Form Fonksiyonları
  const resetNewCariForm = async () => {
    let nextCode = '';
    try {
      const response = await axios.get('/code-template/next-code/CUSTOMER');
      nextCode = response.data?.nextCode || '';
      ...
      console.log('Otomatik kod alınamadı, boş bırakılacak:', error);
      // Hata durumunda sadece logla, form boş olarak açılacak
    }
  ...
  const handleNewCariFormChange = useCallback((field: string, value: any) => {
    setNewCariFormData(prev => ({ ...prev, [field]: value }));
  }, []);

  const handleNewCariAdd = async () => {
    try {
      if (!newCariFormData.unvan || !newCariFormData.unvan.trim()) {
        showSnackbar('Ünvan boş olamaz', 'error');
        return;
      }

      setNewCariLoading(true);
      const dataToSend = prepareDataToSend(newCariFormData);
      ...
      setNewCariLoading(false);
    }
  };

  return (
    <MainLayout>
      {/* Yeni Cari Ekle Formu - Tabbed Slide Menü İçinde */}
      {activeTab === 'yeni-cari' && (
        <Paper
          sx={{
            p: 3,
        ...
        >
          <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box>
              ...
              >
                Yeni müşteri veya tedarikçi bilgilerini girin
              </Typography>
            </Box>
            <Button
              variant="outlined"
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <CariForm
              data={newCariFormData}
              onChange={handleNewCariFormChange}
              onCityChange={handleCityChange}
              availableDistricts={availableDistricts}
              satisElemanlari={satisElemanlari}
            />
            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', pt: 2, borderTop: '1px solid var(--border)' }}>
              <Button
                onClick={() => {
                  setActiveTab('cari-liste');
                  setShowNewCariForm(false);
              ...
              <Button
                variant="contained"
                onClick={handleNewCariAdd}
                disabled={newCariLoading}
                sx={{
              ...
              >
                {newCariLoading ? 'Kaydediliyor...' : 'Kaydet'}
              </Button>
            </Box>
          </Box>
        </Paper>
      ...
      {/* Cari Listesi - Sadece cari-liste sekmesinde göster */}
      {activeTab === 'cari-liste' && (
        <>
          <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box>
              <Typography
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <Button
                ...
                onClick={() => router.push('/account/fatura-kapatma')}
                sx={{
                  borderColor: 'var(--secondary)',
                  color: 'var(--secondary)',
                  textTransform: 'none',
