...
export default function YeniSatinAlmaSiparisiPage() {
  ...
  const generateSiparisNo = async () => {
    try {
      ...
      const lastSiparis = siparisler[0];
      const lastNo = lastSiparis ? parseInt(lastSiparis.siparisNo.split('-')[2] || '0') : 0;
      const newNo = (lastNo + 1).toString().padStart(3, '0');
      setFormData(prev => ({
        ...prev,
        siparisNo: `SAS-${new Date().getFullYear()}-${newNo}`,
      }));
    } catch (error) {
      console.error('Sipariş numarası oluşturulurken hata:', error);
      // Hata durumunda varsayılan numara
      setFormData(prev => ({
        ...prev,
        siparisNo: `SAS-${new Date().getFullYear()}-001`,
      }));
    }
  };
  ...
  const handleKalemChange = (index: number, field: keyof SiparisKalemi, value: any) => {
    setFormData(prev => {
  ...
  const calculateKalemTutar = (kalem: SiparisKalemi) => {
    const araToplam = kalem.miktar * kalem.birimFiyat;
  ...
  const handleSave = async () => {
    try {
      ...
      showSnackbar('Satın alma siparişi başarıyla oluşturuldu', 'success');
      ...
      console.error('Satın alma sipariş kaydetme hatası:', error);
        ...
        <Stack spacing={3}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <TextField
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, siparisNo: e.target.value }))}
              required
            />
            <TextField
              sx={{ flex: '1 1 200px' }}
              ...
              onChange={(_, newValue) => {
                setFormData(prev => {
                  ...
                  if (newValue?.vadeSuresi && newValue.vadeSuresi > 0) {
                    const siparisDate = new Date(prev.tarih);
                    const vadeDate = new Date(siparisDate);
                  ...
                  <Box component="li" key={key} {...otherProps}>
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {option.cariKodu} - {option.tip === 'MUSTERI' ? 'Müşteri' : option.tip === 'TEDARIKCI' ? 'Tedarikçi' : 'Her İkisi'}
                      </Typography>
                    </Box>
                  </Box>
                );
