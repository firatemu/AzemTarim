...
export function PurchasePriceDialog({ open, stok, onClose, onPriceCreated }: PurchasePriceDialogProps) {
  ...
  const fetchPriceCards = useCallback(async () => {
    ...
    try {
      ...
      console.error('Satın alma fiyat kartları alınamadı', error);
  ...
  useEffect(() => {
    if (open) {
      ...
      resetForm(stok?.alisFiyati ?? undefined);
    }
  }, [open, fetchPriceCards, resetForm, stok?.alisFiyati]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, severity: 'success', message: 'Yeni satın alma fiyatı kaydedildi.' });
      resetForm();
      await fetchPriceCards();
      onPriceCreated?.(price);
    } catch (error: any) {
      console.error('Satın alma fiyat kartı kaydedilemedi', error);
      ...
      <DialogTitle component="div" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Ürünün satın alma fiyat geçmişini görüntüleyin ve yeni fiyat ekleyin.
          </Typography>
        </Box>
