...
describe('B2BFifoService', () => {
  let service: B2BFifoService;

  beforeEach(() => {
    service = new B2BFifoService();
  });
  ...
  it('payment before invoice in array still applies FIFO by date sort', () => {
    ...
    expect(r.movements.find((m) => m.id === 'inv1')?.remainingInvoiceDebit?.toString()).toBe(
      '70',
    );
  });
      ...
      [
        {
          ...
          date: new Date('2025-01-01'),
          type: B2BMovementType.INVOICE,
          debit: new Prisma.Decimal('99.99'),
          credit: 0,
        },
      ],
      10,
    ...
    const r = service.calculateFifo(
      [
        ...
        },
        {
          ...
          type: B2BMovementType.PAYMENT,
          debit: 0,
          credit: 80, // More than first invoice (50)
        },
      ],
      0,
      new Date('2025-01-20'),
  ...
  it('all invoices fully paid: overdueAmount is 0', () => {
    const r = service.calculateFifo(
      ...
      30,
      new Date('2025-03-01'), // Well after due dates
    );
    expect(r.summary.overdueAmount.toString()).toBe('0');
    expect(r.summary.balance.toString()).toBe('0');
    expect(r.summary.oldestOverdueDate).toBeNull();
