...
export class QuickInvoiceService {
  ...
  async getTokenStatus() {
    ...
    return {
      ...
      message: hasToken ? (isValid ? 'Token exists and is valid' : 'Token exists but expired') : 'Token not found',
    ...
    try {
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        ...
        if (loginOutput.Token) {
          ...
          this.logger.error('Token field not found in login response');
        ...
        if (response.data.Token) {
          ...
          this.logger.error('Token field not found in login response (object format)');
          throw new Error('Unexpected login response format - Token not found');
        }
      } else {
  ...
  async autoLogin() {
    try {
      ...
      // If hash values are not found, throw error
  ...
  async getIncoming(
    ...
    try {
      ...
      if (!this.cachedToken || !this.isTokenValid()) {
        ...
        try {
          ...
          return {
            success: false,
            documents: [],
            message: 'Token not found and auto login failed. You must login first.',
            total: 0,
            isSucceeded: false,
          };
        }
      ...
      // Tarihleri UTC'ye çevir ve saat bilgisini sıfırla (sadece date kısmı)
      ...
      // SOAP için date formatı: ISO 8601 string veya Date nesnesi
      ...
      return new Promise((resolve, reject) => {
        // SOAP için date formatı: C# DateTime formatına uygun olmalı
        // strong-soap Date nesnesini otomatik olarak SOAP DateTime formatına çevirir
        // Ancak date formatını kontrol etmek için logluyoruz
        ...
        client.GetDocumentList(
          ...
          (err: any, result: any) => {
            ...
            if (soapResult?.GetDocumentListResult?.documents) {
              ...
              // Tüm keys'leri logla
              this.logger.warn('🔍 Documents field not found. soapResult keys:', Object.keys(soapResult || {}));
            ...
            if (documents === null || documents === undefined) {
              ...
              // Object statusu - strong-soap bazen tek item'ı object olarak döndürebilir
  ...
  async getDocumentContent(uuid: string, type: string = 'XML') {
    try {
      ...
      // #region agent log
      fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:580', message: 'GetDocumentFile SOAP call', data: { uuid, hasToken: !!this.cachedToken }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
      // #endregion

      return new Promise((resolve, reject) => {
        // #region agent log
        fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:586', message: 'GetDocumentFile params', data: { uuid, paramFormat: 'UUID/Type' }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
        // #endregion
        // GetDocumentFile parametreleri: AppType (1 = Gelen e-Invoice), Uuid, Tur (opsiyonel: 'XML', 'HTML' veya 'PDF')
        const params: any = {
          AppType: 1, // 1 = Gelen e-Invoice (incoming)
        ...
        // #region agent log
        fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:595', message: 'GetDocumentFile params before call', data: { params, uuid }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
        // #endregion
        ...
        client.GetDocumentFile(
          ...
          (err: any, result: any) => {
            // #region agent log
            fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:592', message: 'GetDocumentFile SOAP response', data: { hasError: !!err, hasResult: !!result, resultType: typeof result }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
            // #endregion

            if (err) {
              this.logger.error('GetDocumentFile hatası:', JSON.stringify(err, null, 2));
              // #region agent log
              fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:597', message: 'GetDocumentFile error', data: { error: err?.message || JSON.stringify(err) }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
              // #endregion
              reject(err);
              return;
            }
            ...
            // #region agent log
            fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:605', message: 'GetDocumentFile soapResult keys', data: { soapResultKeys: Object.keys(soapResult || {}), hasGetDocumentFileResult: !!result?.GetDocumentFileResult }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
            // #endregion
            ...
            // #region agent log
            fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:625', message: 'GetDocumentFile content search result', data: { foundContent: !!documentContent, soapResultType: typeof soapResult }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
            // #endregion
            ...
            if (!isSucceeded) {
              ...
              // #region agent log
              fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:625', message: 'GetDocumentFile failed', data: { message, isSucceeded }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
              // #endregion
            ...
            if (!documentContent) {
              this.logger.warn('GetDocumentFile: DocumentContent field not found. Response keys:', Object.keys(soapResult || {}));
              // #region agent log
              fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:633', message: 'GetDocumentFile content not found', data: { soapResultKeys: Object.keys(soapResult || {}) }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
              // #endregion
              reject(new Error('Document content not found'));
              return;
            }

            // #region agent log
            fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:639', message: 'GetDocumentFile success', data: { contentLength: documentContent.length }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
            // #endregion
      ...
      // #region agent log
      fetch('http://localhost:7244/ingest/fde0823c-7edc-4232-a192-3b97a49bcd3d', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'quick-invoice.service.ts:653', message: 'GetDocumentFile exception', data: { errorMessage: error instanceof Error ? error.message : String(error) }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run3', hypothesisId: 'D' }) }).catch(() => { });
      // #endregion
  ...
  async getUrnConfig() {
    return {
      gb: {
        urn: process.env.HIZLI_GB_URN || 'urn:mail:defaultgb@quick-invoicebilisimteknolojileri.net',
        default: process.env.HIZLI_GB_URN || 'urn:mail:defaultgb@quick-invoicebilisimteknolojileri.net',
      },
      pk: {
