...
Object.defineProperty(exports, "EInvoiceBuilderService", {
    enumerable: true,
    get: function() {
        return EInvoiceBuilderService;
    }
});
...
let EInvoiceBuilderService = class EInvoiceBuilderService {
    build(input) {
        const { invoice, items, account, supplier } = input;
        const uuid = invoice.eInvoiceEttn || (0, _uuid.v4)();
        const documentId = invoice.invoiceNo;
        const documentDate = this.formatDate(invoice.date);
        const supplierVkn = supplier?.taxNumber;
        if (!supplierVkn) {
            throw new _common.BadRequestException('Tenant vergi numarası (supplier taxNumber) eksik');
        }
        const customerIdentifier = account.taxNumber || account.nationalId;
        if (!customerIdentifier) {
            throw new _common.BadRequestException('Cari için VKN/TCKN zorunludur');
        }
        // e-Dönüşüm: Senaryo ve Invoice Tipi (dinamik)
        const profileId = invoice.eScenario || 'TICARI_FATURA'; // TEMEL_FATURA | TICARI_FATURA | KAMU_FATURASI
        const invoiceTypeCode = invoice.eInvoiceType || 'SALE'; // SALES | IADE | TEVKIFAT | ISTISNA | OZEL_MATRAH | IHRAC_KAYITLI
        const buyerAlias = invoice.gibAlias || null;
        const xmlLines = items.map((item, index)=>{
...
EInvoiceBuilderService = _ts_decorate([
    (0, _common.Injectable)()
], EInvoiceBuilderService);

//# sourceMappingURL=einvoice-builder.service.js.map