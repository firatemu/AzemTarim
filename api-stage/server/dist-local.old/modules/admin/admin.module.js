"use strict";
Object.defineProperty(exports, "__esModule", {
    value: true
});
Object.defineProperty(exports, "AdminModule", {
    enumerable: true,
    get: function() {
        return AdminModule;
    }
});
const _common = require("@nestjs/common");
const _tenantpurgeservice = require("./tenant-purge.service");
const _admintenantcontroller = require("./admin-tenant.controller");
const _adminlogscontroller = require("./admin-logs.controller");
const _prismamodule = require("../../common/prisma.module");
const _storagemodule = require("../storage/storage.module");
function _ts_decorate(decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for(var i = decorators.length - 1; i >= 0; i--)if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
}
let AdminModule = class AdminModule {
};
AdminModule = _ts_decorate([
    (0, _common.Module)({
        imports: [
            _prismamodule.PrismaModule,
            _storagemodule.StorageModule
        ],
        controllers: [
            _admintenantcontroller.AdminTenantController,
            _adminlogscontroller.AdminLogsController
        ],
        providers: [
            _tenantpurgeservice.TenantPurgeService
        ],
        exports: [
            _tenantpurgeservice.TenantPurgeService
        ]
    })
], AdminModule);

//# sourceMappingURL=admin.module.js.map