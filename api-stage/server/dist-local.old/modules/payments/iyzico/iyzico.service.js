...
Object.defineProperty(exports, "IyzicoService", {
    enumerable: true,
    get: function() {
...
let IyzicoService = class IyzicoService {
            ...
            case 'PAYMENT_SUCCESS':
                ...
                await this.prisma.$transaction(async (tx)=>{
                    ...
                    if (payment.subscription) {
                        ...
                        if (tenant && tenant.tenantType === 'INDIVIDUAL' && tenant.status !== 'ACTIVE') {
                            ...
                            for (const t of templates){
                                const exists = await tx.codeTemplate.findFirst({
                                    where: {
                                        tenantId,
                                        module: t.module
                                    }
    ...
    async handleCallback(token) {
        ...
        this.logger.log('Handling callback', {
            token
        });
        ...
        if (!payment) {
            this.logger.warn('Payment not found for callback token', {
                token
            });
            return {
                success: false,
    ...
    constructor(configService, prisma, tenantResolver){
        this.configService = configService;
        this.prisma = prisma;
        this.tenantResolver = tenantResolver;
        this.logger = new _common.Logger(IyzicoService.name);
        this.apiKey = this.configService.get('IYZICO_API_KEY') || '';
        this.secretKey = this.configService.get('IYZICO_SECRET_KEY') || '';
        this.baseUrl = this.configService.get('IYZICO_BASE_URL') || 'https://api.iyzipay.com';
    }
};
IyzicoService = _ts_decorate([
